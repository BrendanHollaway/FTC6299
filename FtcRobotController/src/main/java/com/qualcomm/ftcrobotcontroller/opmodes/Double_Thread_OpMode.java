 ...
 */
public class Double_Thread_OpMode extends OpMode{

    Thread armThread;
    Thread driveThread;
    Arm_BufferedRunnable armRunnable;
    Drive_BufferedRunnable driveRunnable;

    @Override
    public void init() {
        armRunnable = new Arm_BufferedRunnable(hardwareMap);
        armRunnable.passNewJoystickInfo(gamepad1, gamepad2);
        armThread = new Thread(armRunnable);

        driveRunnable = new Drive_BufferedRunnable(hardwareMap);
